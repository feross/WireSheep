...
HttpPacket::HttpPacket(string from, string to)
  : m_from(from), m_to(to), m_complete(false), m_id(curr_id++)
{
...
bool HttpPacket::isResponse()
{
  return m_parser.type == HTTP_RESPONSE;
}
...
{ 
  ...
  if(isResponse()) {
    ...
    if(encoding == "gzip") {
      ...
      m_filterstream.push(zlib_decompressor());
